    ...
    <>
      <header className="fixed top-0 z-50 w-full border-b border-white/5 bg-[#08090B]/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-6 sm:py-5">
          <a
            href="#top"
            className="flex items-center gap-3 text-xs font-semibold tracking-[0.28em] text-[#F3F1EA] sm:text-sm"
          >
          ...
          <nav className="flex items-center gap-3 md:gap-6">
            {links.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className={`${link.className} text-sm text-[#A7A29A] transition-colors hover:text-[#F3F1EA]`}
              >
                {link.label}
              </a>
            ))}
            ...
            <button
              ...
              className="rounded-full bg-orange-500/10 px-4 py-2 text-sm font-semibold text-orange-300 transition hover:bg-orange-500 hover:text-white"
            >
              Contact
            </button>
          </nav>
