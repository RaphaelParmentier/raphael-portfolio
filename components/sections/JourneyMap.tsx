...
const principles = ["Observation", "Evidence", "Systems", "Scale", "Clarity"];
...
export default function JourneyMap() {
  ...
  return (
    <section
      id="journey"
      className="relative z-10 mx-auto max-w-[112rem] px-5 py-24 sm:px-6 lg:py-32 2xl:py-40 2xl:px-10"
    >
      <SectionHeader
        eyebrow="Journey Map"
        title="A trajectory built around evidence, systems and clarity."
        description="The profile is not a straight line from statistics to AI. It is a progression from observing complex systems, analysing them, building tools, scaling workflows and teaching what works."
      />

      <div className="mt-12 grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-stretch 2xl:mt-16 2xl:gap-12">
        <div className="rounded-[2.5rem] border border-slate-800 bg-slate-950/70 p-4 shadow-2xl shadow-orange-500/5 backdrop-blur sm:p-5 2xl:p-7">
            ...
            {journeySteps.map((step, index) => {
              ...
              return (
                <button
                  ...
                  className={`group min-w-[250px] rounded-2xl border p-4 text-left transition lg:min-w-0 xl:p-5 2xl:p-6 ${
                    isActive
          ...
          <div className="mt-6 hidden rounded-[2rem] border border-orange-500/20 bg-orange-500/[0.07] p-5 lg:block 2xl:mt-8 2xl:p-7">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2 text-sm font-semibold uppercase tracking-[0.16em] text-slate-500 2xl:text-base">
              {principles.map((principle, index) => (
                <span key={principle} className="flex items-center gap-2">
                  <span
                    className={
                      index <= activeIndex ? "text-orange-200" : "text-slate-600"
                    }
                  >
                    {principle}
                  </span>
            ...
            <p className="mt-5 text-sm leading-7 text-orange-100/80 2xl:text-base 2xl:leading-8">
              Each step adds one layer to the same operating logic: turning
              complex information into decisions people can use.
            </p>
          </div>
        </div>

        <div className="relative overflow-hidden rounded-[2.5rem] border border-slate-800 bg-slate-950/75 p-6 shadow-2xl shadow-orange-500/10 sm:p-8 lg:p-10 2xl:p-12">
            ...
            >
              <div className="flex flex-col gap-4 border-b border-slate-800 pb-7 sm:flex-row sm:items-start sm:justify-between 2xl:pb-9">
                <div>
                  ...
                  <p className="mt-3 text-sm text-slate-500 2xl:text-lg">
                    {activeStep.period}
                  </p>
                </div>

                <span className="w-fit rounded-full border border-slate-700 bg-slate-900/70 px-3 py-1 text-xs text-slate-300 2xl:text-sm">
